...
async def post_alpaca_order(buy_price, sell_price, side):
    ...
    try:
        if side == "buy":
            ...
            buy_limit_order = trading_client.submit_order(order_data=limit_order_data)
            buy_order_price = buy_price
            sell_order_price = sell_price
            # buy_order = True
            logger.info(
                "Buy Limit Order placed for ETH/USD at : {0}".format(
                    buy_limit_order.limit_price
                )
            )
            ...
            # sell_order = True
            logger.info(
                "Sell Limit Order placed for ETH/USD at : {0}".format(
                    sell_limit_order.limit_price
                )
            )
